...
public class GeneratePlanServiceImpl implements GeneratePlanService {
  ...
  public List<BorrowerPaymentsDTO> generatePlan(LoanDataDto loanDataDto) {
    ...
    dtoList.add(
        repaymentPlan(
            annuity, loanDataDto.getNominalRate(), loanDataDto.getLoanAmount(), startDate));
  ...
  private BorrowerPaymentsDTO repaymentPlan(
      double annuity, double rate, double initialOutstandingPrincipal, LocalDate date) {
    ...
    log.info(
        "date: {}, interest: {}, principal: {}, borrowerPaymentAmount: {}, remainingOutstandingPrincipal: {}",
        date,
        interest,
        principal,
  ...
  private double calculateInterest(double interestRate, double initialOutstandingPrincipal) {
    return roundValues(
        interestRate * 0.01 * daysInMonth * initialOutstandingPrincipal / daysInYear);
  }

  private double calculatePrincipal(
      double annuity, double interest, double initialOutstandingPrincipal) {
  ...
  private double calculateRemainingOutstandingPrincipal(
